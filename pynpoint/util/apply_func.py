...
def align_image(
    ...
    ref_images_shape: Tuple[int, int, int],
) -> np.ndarray:
    ...
    for i in range(num_references):
        if subframe is None:
            ...
            tmp_offset, _, _ = phase_cross_correlation(
                sub_ref, sub_in, normalization=None, upsample_factor=accuracy
            )
...
def fit_2d_function(
    ...
    @typechecked
    def gaussian_2d(
        grid: Union[Tuple[np.ndarray, np.ndarray], np.ndarray],
        x_center: float,
        y_center: float,
        fwhm_x: float,
        fwhm_y: float,
        ...
        sigma_x = fwhm_x / np.sqrt(8.0 * np.log(2.0))
        sigma_y = fwhm_y / np.sqrt(8.0 * np.log(2.0))

        a_gauss = 0.5 * (
            (np.cos(theta) / sigma_x) ** 2 + (np.sin(theta) / sigma_y) ** 2
        )
        b_gauss = 0.5 * (
            (np.sin(2.0 * theta) / sigma_x**2) - (np.sin(2.0 * theta) / sigma_y**2)
        )
        c_gauss = 0.5 * (
        ...
        gaussian = offset + amp * np.exp(
            -(a_gauss * x_diff**2 + b_gauss * x_diff * y_diff + c_gauss * y_diff**2)
        )
    ...
    def moffat_2d(
        ...
        if 2.0 ** (1.0 / beta) - 1.0 < 0.0:
            alpha_x = np.nan
            alpha_y = np.nan

        else:
            alpha_x = 0.5 * fwhm_x / np.sqrt(2.0 ** (1.0 / beta) - 1.0)
            alpha_y = 0.5 * fwhm_y / np.sqrt(2.0 ** (1.0 / beta) - 1.0)
        ...
        if alpha_x == 0.0 or alpha_y == 0.0:
            ...
            a_moffat = (np.cos(theta) / alpha_x) ** 2.0 + (
                np.sin(theta) / alpha_y
            ) ** 2.0
            b_moffat = (np.sin(theta) / alpha_x) ** 2.0 + (
                np.cos(theta) / alpha_y
            ) ** 2.0
            c_moffat = (
                2.0
                * np.sin(theta)
                * np.cos(theta)
                * (1.0 / alpha_x**2.0 - 1.0 / alpha_y**2.0)
            )
        ...
        moffat = offset + amp / (1.0 + a_term + b_term + c_term) ** beta
    ...
    if mask_out_port is not None:
        ...
        mask[(rr_ap < mask_radii[0]) | (rr_ap > mask_radii[1])] = 0.0

        mask_out_port.append(mask, data_dim=3)

    if sign == "negative":
        image = -1.0 * image + np.abs(np.min(-1.0 * image))
...
def crop_rotating_star(
    ...
    starpos = locate_star(
        image=image, center=tuple(position), width=search_size, fwhm=filter_size
    )
...
@typechecked
def photometry(
    image: np.ndarray, im_index: int, aperture: Union[Aperture, List[Aperture]]
) -> np.ndarray:
    ...
    return np.array(
        aperture_photometry(image, aperture, method="exact")["aperture_sum"]
    )


@typechecked
def image_stat(
    image_in: np.ndarray, im_index: int, indices: Optional[np.ndarray]
) -> np.ndarray:
...
def dwt_denoise_line_in_time(
    ...
    coef = pywt.wavedec(
        signal_in, wavelet=wavelet_conf.m_wavelet, level=None, mode=padding
    )
    ...
    denoised[1:] = (
        pywt.threshold(i, value=threshold, mode=threshold_mode) for i in denoised[1:]
    )
...
@typechecked
def normalization(image_in: np.ndarray, im_index: int) -> np.ndarray:
...
def calc_fast_convolution(
    ...
    if (
        ((tmp_s[0] == 0) and (tmp_s[1] == 0))
        or ((tmp_s[0] == N[0] / 2) and (tmp_s[1] == 0))
        or ((tmp_s[0] == 0) and (tmp_s[1] == N[1] / 2))
        or ((tmp_s[0] == N[0] / 2) and (tmp_s[1] == N[1] / 2))
    ):
        ...
        for m in range(0, N[0], 1):
            for j in range(0, N[1], 1):
                new[m, j] = (
                    F_roof_tmp * W[m - tmp_s[0], j - tmp_s[1]]
                    + np.conjugate(F_roof_tmp)
                    * W[(m + tmp_s[0]) % N[0], (j + tmp_s[1]) % N[1]]
                )

    if (
        ((tmp_s[0] == N[0] / 2) and (tmp_s[1] == 0))
        or ((tmp_s[0] == 0) and (tmp_s[1] == N[1] / 2))
        or ((tmp_s[0] == N[0] / 2) and (tmp_s[1] == N[1] / 2))
    ):  # causes problems, unknown why
...
def bad_pixel_interpolation(
    image_in: np.ndarray, bad_pixel_map: np.ndarray, iterations: int
) -> np.ndarray:
    """
    ...
    while iteration < iterations:
        ...
        tmp_s = np.unravel_index(
            np.argmax(abs(tmp_G.real[:, 0 : N[1] // 2])), (N[0], N[1] // 2)
        )
        ...
        # conjugate line exists
            ...
            a = np.power(np.abs(W[(0, 0)]), 2)
            b = (
                np.power(np.abs(W[(2 * tmp_s[0]) % N[0], (2 * tmp_s[1]) % N[1]]), 2.0)
                + 0.01
            )
            c = a - b
...
def image_interpolation(
    image_in: np.ndarray, im_index: int, iterations: int, bad_pixel_map: np.ndarray
) -> np.ndarray:

    return bad_pixel_interpolation(image_in, bad_pixel_map, iterations)


@typechecked
def replace_pixels(
    image: np.ndarray, im_index: int, index: np.ndarray, size: int, replace: str
) -> np.ndarray:
    ...
    for _, item in enumerate(index):
        im_tmp = im_mask[
            item[0] - size : item[0] + size + 1, item[1] - size : item[1] + size + 1
        ]
...
@typechecked
def bad_pixel_sigma_filter(
    image_in: np.ndarray,
    im_index: int,
    box: int,
    sigma: float,
    iterate: int,
    map_out_port: Optional[OutputPort],
) -> np.ndarray:
    ...
    while iterate > 0:
        ...
        out_image, bad_pixel_map = sigma_filter(
            dev_image,
            var_image,
            mean_image,
            source_image,
