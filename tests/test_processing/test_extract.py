...
class TestExtract:
    ...
    def test_read_data(self) -> None:
        ...
        assert np.sum(data) == pytest.approx(
            11.012854046962481, rel=self.limit, abs=0.0
        )
        assert data.shape == (10, 21, 21)

        self.pipeline.set_attribute(
            "binary", "PARANG", -1.0 * np.linspace(0.0, 180.0, 10), static=False
        )
    ...
    def test_extract_position_none(self) -> None:
        ...
        assert (
            ...
            == "Can not store the attribute 'INSTRUMENT' because "
            "the dataset 'index' does not exist."
        )

        assert (
            warning[1].message.args[0]
            == "Can not store the attribute 'PIXSCALE' because "
            "the dataset 'index' does not exist."
        )
