...
class TestPypeline:
    ...
    def setup_class(self) -> None:
        ...
        header["HIERARCH ESO ADA POSANG"] = 10.0
    ...
    def test_create_default_config(self) -> None:
        ...
        assert (
            warning[0].message.args[0] == "Configuration file not found. Creating "
    ...
    def test_create_none_config(self) -> None:
        ...
        assert attribute == pytest.approx(0.0, rel=self.limit, abs=0.0)
    ...
    def test_create_pipeline_path_missing(self) -> None:
        ...
        assert (
            str(error.value)
            == "The folder that was chosen for the working place does not "
        ...
        assert (
            str(error.value)
            == "The folder that was chosen for the input place does not "
            "exist: " + self.test_dir + "none_dir/."
        )
        ...
        assert (
            str(error.value)
            == "The folder that was chosen for the output place does not "
            "exist: " + self.test_dir + "none_dir/."
        )
    ...
    def test_create_pipeline_existing_database(self) -> None:
        ...
        assert np.mean(data) == pytest.approx(
            1.1824138000882435e-05, rel=self.limit, abs=0.0
        )
    ...
    def test_add_module(self) -> None:
        ...
        module = FitsReadingModule(name_in="read1", input_dir=None, image_tag="im_arr1")

        assert pipeline.add_module(module) is None

        module = FitsReadingModule(
            name_in="read2", input_dir=self.test_dir, image_tag="im_arr2"
        )
        ...
        assert (
            ...
            == "Names of pipeline modules that are added to the "
        ...
        module = BadPixelSigmaFilterModule(
            name_in="badpixel", image_in_tag="im_arr1", image_out_tag="im_out"
        )

        assert pipeline.add_module(module) is None

        module = FitsWritingModule(
            name_in="write1", file_name="result.fits", data_tag="im_arr1"
        )
    ...
    def test_run_module_wrong_tag(self) -> None:
        ...
        module = FitsWritingModule(
            name_in="write", file_name="result.fits", data_tag="im_list"
        )

        pipeline.add_module(module)

        module = BadPixelSigmaFilterModule(
            name_in="badpixel", image_in_tag="im_list", image_out_tag="im_out"
        )
        ...
        assert (
            str(error.value)
            == "Pipeline module 'badpixel' is looking for data under a "
        ...
        assert (
            str(error.value)
            == "Pipeline module 'write' is looking for data under a tag "
    ...
    def test_remove_module(self) -> None:
        ...
        assert (
            warning[0].message.args[0] == "Pipeline module 'test' is not found in the "
    ...
    def test_list_attributes(self) -> None:
        ...
        assert attr_dict["PARANG_START"] == [10.0]
    ...
    def test_set_and_get_attribute(self) -> None:
        ...
        pipeline.set_attribute(
            "images", "PARANG", np.arange(1.0, 11.0, 1.0), static=False
        )

        attribute = pipeline.get_attribute("images", "PIXSCALE", static=True)
        assert attribute == pytest.approx(0.1, rel=self.limit, abs=0.0)

        attribute = pipeline.get_attribute("images", "PARANG", static=False)
        assert attribute == pytest.approx(
            np.arange(1.0, 11.0, 1.0), rel=self.limit, abs=0.0
        )

        pipeline.set_attribute(
            "images", "PARANG", np.arange(10.0, 21.0, 1.0), static=False
        )

        attribute = pipeline.get_attribute("images", "PARANG", static=False)
        assert attribute == pytest.approx(
            np.arange(10.0, 21.0, 1.0), rel=self.limit, abs=0.0
        )
    ...
    def test_delete_not_found(self) -> None:
        ...
        assert (
            warning[0].message.args[0] == "Dataset 'images' not found in the database."
        )
        assert (
            warning[1].message.args[0]
            == "Attributes of 'images' not found in the database."
        )
